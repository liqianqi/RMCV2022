...
public:
    // 获得电控发过来的数据
    void getPrivateData(float &v0, Armor &armor, GimbalPose &current_gimbalpose,bool is_have_data){
        this->v0 = v0*1000;
...
private:
    /// 一些状态变量
    Kalman_Filter kalaman,v_kalaman;
    AntiGyro antigyro;
    tuple<long ,long> two;
...
public:
    ...
    * polylines()绘制曲线文件
    */
    static void drawCurveData(cv::Point3f point);

    /**
    * 实时绘制曲线
    * polylines()绘制曲线文件
    */
    static void drawCurveData1(cv::Point3f point);
