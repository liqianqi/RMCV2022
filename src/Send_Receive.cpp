...
void DataControler::sentData(int fd,VisionData data){
    ...
    unsigned char send_bytes[23] = {0};

    if(is_start_time_stamp_){
        send_bytes[0] = 0xee;
        send_bytes[22] = 0xff;
...
//    CLEAR_BIT((data.OnePointFive),8);
  ...
  //   SET_BIT((data.OnePointFive),2);
    ...
    send_bytes[14] = data.pitch_w_rads.c[0];
    send_bytes[15] = data.pitch_w_rads.c[1];
    send_bytes[16] = data.pitch_w_rads.c[2];
    send_bytes[17] = data.pitch_w_rads.c[3];

    send_bytes[18] = data.yaw_w_rads.c[0];
    send_bytes[19] = data.yaw_w_rads.c[1];
    send_bytes[20] = data.yaw_w_rads.c[2];
    send_bytes[21] = data.yaw_w_rads.c[3];


    write(fd, send_bytes, 23);
